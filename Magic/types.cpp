...
Types::Types() 
{
	...
	{
		std::string message = "Types::Types(): unable to load types from DB.";
		throw std::runtime_error(message.c_str());
	}
	/*
	if (!create()) {
		std::string message = "Types::Types(): unable to load types from DB. File not found or errors found: " + DATA_PATH + TYPES_FILE;
		throw std::runtime_error(message.c_str());
	} */
}
...
bool Types::loadFromDB(std::vector<std::string> &messages)
{
	bool valid = true;
...
bool Types::loadCardTypes(std::vector<std::string> &messages)
{
	bool valid = true;
	...
	request.setDBName(database.DATABASE_NAME);
	request.addTableName(database.CARD_SETS_TABLE_NAME);

	std::vector<SQLDBRow> rows = database.retrieveRows(request);

	if (rows.size() == 0)
	{
		messages.emplace_back("Types::loadCardTypes() no card type rows found");
		valid = false;
	}
	else
	{
